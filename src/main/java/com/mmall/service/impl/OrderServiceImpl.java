...
public class OrderServiceImpl implements IOrderService{
    ...
    public ServerResponse<String> cancel(Integer userId,Long orderNo){
        Order order  = orderMapper.selectByuserIdAndorderNo(userId.toString(),orderNo.toString());
        if(order == null){
    ...
    @Override
    public ServerResponse pay(String userId, String orderNo, String path) {
     Map map = new HashMap();
     Order order = orderMapper.selectByuserIdAndorderNo(userId,orderNo);
     if(order == null){
     return ServerResponse.createByError("无此订单信息");
     }
        ...
        String subject = new StringBuilder().append("bright扫码支付,订单号:").append(order.getOrderNo().toString()).toString();
        ...
        String body = new StringBuffer().append("订单号:").append(order.getOrderNo()).append(",购买商品共").append(totalAmount).append("元").toString();
        ...
        //获取订单商品详情
        List<OrderItem>   list  = orderItemMapper.selectOrderNoAnduserId(Long.valueOf(userId),Long.valueOf(orderNo));
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                try {
                    FTPUtil.fileUpload(Lists.newArrayList(targetFile));
                } catch (IOException e) {
                    log.error("上传二维码异常",e);
                }
    ...
    public void closeOrder(int hour) {
        ...
        // 获取支付超时的订单
        List<Order>  orderList =   orderMapper.selectOrderByCreateTime(
                Const.OrderStatusEnum.NO_PAY.getCode(),DateTimeUtil.dateToStr(closeTime));
        for (Order order : orderList) {
            // 获取详情
            List<OrderItem> orderItemList =
                    orderItemMapper.getByOrderNo(order.getOrderNo());
            for (OrderItem orderItem : orderItemList) {
                // for update where条件要指定主键，否则会锁表
               Integer stock =
                       productMapper.selectStockById(orderItem.getProductId());
               // 如果为null，不做处理
               if(null == stock){
                   continue;
               }
